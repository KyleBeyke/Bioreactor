...
Commands sent to the Pico are properly terminated with '\n\r' for correct processing.
"""
        ...
        while True:
            ...
            try:
                if ser.in_waiting > 0:
                    ...
                    # Assume the incoming serial data is in the format "SENSOR DATA:timestamp,CO2,temperature,humidity,pressure"
                    # Parse the CO2 value
                    if serial_data.startswith("SENSOR DATA:"):
                        ...
                        if len(data_parts) >= 6:  # For the usual 6 parts
                            ...
                            logging.error(f"Malformed sensor data
