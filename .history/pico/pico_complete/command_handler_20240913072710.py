...
class CommandHandler:
    ...
    def handle(self, command):
        ...
        try:
            ...
            # Set the target temperature for the heater (e.g., "SET_HEATER_TEMP,45")
            if command.startswith("SET_HEATER_TEMP"):
                ...
                self.heater_controller.pid_controller.setpoint = temp  # Update the PID controller setpoint

            # Set the maximum heater duty cycle (e.g., "SET_HEATER_DUTY,30")
                ...
                self.heater_controller.max_duty_cycle = duty_cycle  # Update the heater's max duty cycle cap

            # Turn the heater ON ("HEATER_ON")
            elif command == "HEATER_ON":
                Logger.log_info("Turning heater ON.")
                self.heater_controller.turn_on()  # Activate the heater

            # Turn the heater OFF ("HEATER_OFF")
            elif command == "HEATER_OFF":
                Logger.log_info("Turning heater OFF.")
                self.heater_controller.turn_off()  # Deactivate the heater
